...
# ---- Mobile base closed-loop alignment (copied from picking_box_5) ----
...
def setup_max_open_gripper(robot: Any) -> MaxOpenGripper | None:
    ...
    print_stage("0/13 gripper_home_open", "setting tool voltage")
    if not enable_gripper_tool_voltage(robot):
        return None
    gripper = MaxOpenGripper()
    print_stage("0/13 gripper_home_open", "priming Dynamixel bus after tool voltage")
    gripper.initialize(verbose=True)
    time.sleep(GRIPPER_SETUP_SETTLE_SEC)
    print_stage("0/13 gripper_home_open", "verifying Dynamixel bus")
    if not gripper.initialize(verbose=True):
        return None
    print_stage("0/13 gripper_home_open", "homing both gripper fingers")
    if not gripper.homing():
        return None
    if not gripper.set_open_target():
        return None
    gripper.start()
    print_stage("0/13 gripper_home_open", f"max-open hold started; min={gripper.min_q}, max={gripper.max_q}")
    return gripper
...
def run_mobile_base_yaw_alignment(
    ...
    *,
    ...
    max_center_spread_m: float,
    stage: str = "3/13 mobile_base_yaw_align",
) -> dict[str, Any] | None:
...
def run_mobile_base_alignment(
    ...
    *,
    ...
    stage: str = "4/13 mobile_base_xy_align",
) -> dict[str, Any] | None:
...
def run_mobile_base_combined_servo_alignment(
    ...
    *,
    ...
    vision_frames_needed: int,
    stage: str = "3-4/13 mobile_base_se2_align",
) -> dict[str, Any] | None:
...
def stream_impedance_push_stage(
    ...
    *,
    ...
    def send_progress(progress: float, *, command_hold_time: float) -> float:
        ...
        stream.send_command(
            build_impedance_push_command(
                ...
                dyn_state,
                q0,
                inward=float(target_inward) * clamped,
    ...
    print_stage(
        ...
        f"to inward={float(target_inward):+.3f}m at {1.0 / period:.1f}Hz",
    )
    start = time.monotonic()
    next_send = start
    next_log = start + COMMAND_WAIT_LOG_INTERVAL_SEC
...
def build_impedance_vertical_command(
    dyn_model: Any,
    dyn_state: Any,
    q: Any,
    *,
    ...
    stiffness/damping/torque limit) so the box keeps being HELD while it moves:
    ...
    delta = float(vertical_delta_m)
    T_right_target = offset_translation(T_base2right, 0.0, delta)
    T_left_target = offset_translation(T_base2left, 0.0, delta)

    print(
        f"[{label}] right EEF z: {T_base2right[2, 3]:+.3f} -> {T_right_target[2, 3]:+.3f} m"
...
def build_impedance_lift_command(dyn_model: Any, dyn_state: Any, q: Any) -> Any:
    """Raise both hands by LIFT_HEIGHT along base +z, compliantly."""
    return build_impedance_vertical_command(
        dyn_model,
        dyn_state,
        q,
        vertical_delta_m=+LIFT_HEIGHT,
        label="lift",
    )


def build_impedance_lower_command(dyn_model: Any, dyn_state: Any, q: Any) -> Any:
    """Lower both hands by LIFT_HEIGHT along base -z, compliantly."""
    return build_impedance_vertical_command(
        dyn_model,
        dyn_state,
        q,
...
def wait_for_external_transfer(
    *,
    require_confirm: bool,
    wait_sec: float,
    stage: str = "8/13 external_transfer",
) -> bool:
    """Pause after the first lift while the opposite-table transfer happens."""
    wait = max(0.0, float(wait_sec))
    if wait > 0.0:
        print_stage(stage, f"waiting {wait:.1f}s for external transfer")
        time.sleep(wait)
    if require_confirm:
        print_stage(stage, "waiting for operator confirmation before place/regrasp")
        try:
            input("[transfer] Move to the destination table, then press Enter to place/regrasp: ")
        except EOFError:
            print_stage(stage, "FAILED: stdin closed before transfer confirmation")
            return False
    else:
        print_stage(stage, "confirmation skipped by --no-transfer-confirm")
    return True
...
def perform_place_regrasp_sequence(
    ...
    *,
    ...
    """Place by reversing lift/push, wait, then regrasp and lift again.

    This destination sequence intentionally sends only dual-arm Cartesian
    impedance commands. Mobile base, torso, and head remain wherever the
    external transfer left them.
    """
    print_stage("9/13 place_lower", "building reverse-lift target")
    q = robot.get_state().position
    if not send_stage(
        robot,
        ...
        print_stage("9/13 place_lower", "FAILED; aborting")
        return False

    print_stage("10/13 release_push_reverse", "ramping inward target back to zero")
    q = robot.get_state().position
    if not stream_impedance_push_stage(
        ...
        stage="10/13 release_push_reverse",
    ):
        print_stage("10/13 release_push_reverse", "FAILED; aborting")
        return False

    wait = max(0.0, float(place_wait_sec))
    print_stage("11/13 release_wait", f"waiting {wait:.1f}s before regrasp")
    if wait > 0.0:
        time.sleep(wait)

    print_stage("12/13 regrasp_push", "building ramped target stream")
    q = robot.get_state().position
    if not stream_impedance_push_stage(
        robot,
        dyn_model,
        ...
        ramp_time_sec=push_ramp_time_sec,
        hold_time_sec=PUSH_HOLD_TIME,
        stage="12/13 regrasp_push",
    ):
        print_stage("12/13 regrasp_push", "FAILED; aborting")
        return False

    print_stage("13/13 regrasp_lift", "building target")
    q = robot.get_state().position
    if not send_stage(
        robot,
        ...
        print_stage("13/13 regrasp_lift", "FAILED; aborting")
        return False

    print_stage("13/13 regrasp_lift", "done")
    return True
    ...
    try:
        if not visualize_only and gripper_open:
            ...
            if gripper_device is None:
                print_stage("0/13 gripper_home_open", "FAILED; aborting before arm motion")
                return False
        elif not visualize_only:
            print_stage("0/13 gripper_home_open", "skipped by --no-gripper-open")
        ...
        if box_center_camera_m is None:
            ...
            print_stage("2/13 live_vision", "capturing D405 frames")
            ...
            if mobile_base_align and str(model).lower() == "m":
                ...
                aligned_measurement = run_mobile_base_alignment(
                    ...
                    stage="4/13 mobile_base_xy_align",
                )
            ...
            print_stage("2/13 live_vision", "skipped; using supplied center_top_camera_m")
            q = robot.get_state().position
            camera_to_base = compute_camera_to_base_for_view_rotation(
                view_rotation,
                dyn_model,
        ...
        print_stage("5/13 vision_pre_push", "building target")
        q = robot.get_state().position
        ...
        if not send_stage(
            ...
            print_stage("5/13 vision_pre_push", "FAILED; aborting")
            return done
        print_stage("5/13 vision_pre_push", "reached")

        if not continue_pick:
            done = True
            print_stage("5/13 vision_pre_push", "pre-push-only stop; done=True")
            return done

        print_stage("6/13 inward_push_pick", "building ramped target stream")
        q = robot.get_state().position
        if not stream_impedance_push_stage(
            robot,
            dyn_model,
            ...
            ramp_time_sec=push_ramp_time_sec,
            hold_time_sec=PUSH_HOLD_TIME,
            stage="6/13 inward_push_pick",
        ):
            print_stage("6/13 inward_push_pick", "FAILED; aborting")
            return done

        print_stage("7/13 lift_pick", "building target")
        q = robot.get_state().position
        if not send_stage(
            robot,
            build_impedance_lift_command(dyn_model, dyn_state, q),
            ...
            print_stage("7/13 lift_pick", "FAILED; aborting")
            return done
        print_stage("7/13 lift_pick", "done")
        ...
        if not wait_for_external_transfer(
            require_confirm=transfer_confirm,
        ...
        if gripper_device is not None:
            print_stage("0/13 gripper_home_open", "stopping max-open background loop")
            gripper_device.stop()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "This is combined with x/y alignment when both are enabled."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "In combined SE(2) alignment, suppress x/y translation "
    ...
    parser.add_argument(
        ...
        dest="mobile_base_align",
        action="store_false",
        help="Disable mobile-base alignment and use arm-only vision_pre_push.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--transfer-confirm",
        dest="transfer_confirm",
        action="store_true",
        help="Require Enter confirmation after external transfer before place/regrasp. This is the default.",
    )
    parser.add_argument(
        "--no-transfer-confirm",
        dest="transfer_confirm",
        action="store_false",
        help="Do not wait for Enter after transfer; use with --transfer-wait-sec for automated benches.",
    )
    parser.add_argument(
        "--transfer-wait-sec",
        type=float,
        default=0.0,
        help="Timed wait after first lift before destination place/regrasp.",
    )
    parser.add_argument(
        "--place-wait-sec",
        type=float,
...
if __name__ == "__main__":
    ...
    if args.transfer_wait_sec < 0.0:
        raise SystemExit("--transfer-wait-sec must be non-negative")
    ...
    raise SystemExit(
        ...
        if main(
            ...
            place_regrasp=bool(args.place_regrasp),
            transfer_confirm=bool(args.transfer_confirm),
            transfer_wait_sec=float(args.transfer_wait_sec),
