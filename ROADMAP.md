...
| 6/7 inward_push | 0.7s + FK 물림 검증 | 2026-07-08 실기에서 성공 (gap shrink 7.4cm) |
| → 7/7 전환 | ~0.1s (build→cancel→send) | 실기에서 send까지 성공 |
| 7/7 lift | 0.5s 이동 + 100s hold | FinishCode를 기다리면 100s hold 때문에 stuck처럼 보임 → FK lift gate로 수정, **재검증 필요** |
