...
최근 실기 로그 기준 (2026-07-08 오전):

| 스테이지 | 소요 | 상태 |
|---|---|---|
| 0/7 gripper_home_open | 4.4~6.1s | 병목 (P2에서 다룸) |
| 1/7 ready | 2.27s | 고정 (MINIMUM_TIME=2.0) |
| 2/7 live_vision | 1.6~1.8s | 카메라 오픈 포함 |
| 3-4/7 se2_align (서보) | **1.3~3.7s** | 필터 수정 후 안정적 settle; stationary confirm 생략 |
| → 5/7 전환 | **공백 0** | E-패턴 선점 성공 (실기 확인) |
| 5/7 vision_pre_push | **0.80s** | 새 스트림 첫 명령 + FK 게이트 (FinishCode 대기 제거) |
| → 6/7 전환 | ~0.2s (cancel) | body 스트림이 body 스트림을 선점 못해 cancel 필요 |
| 6/7 inward_push | 0.7s + FK 물림 검증 | 마지막 실기에서 크래시 → 수정 완료, **재검증 필요** |
| → 7/7 전환 | ~0.1s (build→cancel→send) | **재검증 필요** |
| 7/7 lift | 0.5s 이동 + hold | production과 동일 (send_command, FinishCode) |

미커밋 상태: E-패턴 체인 최종본(pre-push 도착 후 cancel, push 예외→재시도,
lift build→cancel→send)이 작업 트리에만 있음. **Jetson은 git으로 동기화하므로
커밋해야 실기 검증 가능.**
...
lift (build → cancel_control → 즉시 robot.send_command, 공백 ~0.1s)
  └ production과 동일한 CartesianImpedance, FinishCode 대기
```
...
3. 기대 로그 순서:
   ...
   - `cancel_control to release push hold; sending lift` → `finish_code=FinishCode.Ok`
4. LED 관찰: 초록 유지 구간과 파란 blip 위치/길이 (pre-push→push, push→lift 두 곳,
   각 0.5초 이내여야 함).
5. `FAILED`/`fallback` 메시지가 있으면 그 단계의 가정이 깨진 것 — 로그 확보.

### P1 — push→lift 공백 0 만들기 (가장 유망한 다음 개선)
...
### P1.5 — pre-push→push 공백 0 (난이도 높음, 선택)
