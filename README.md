...
파일이 없으면 Jetson에 맞는 Stereolabs ZED SDK를 먼저 설치해야 합니다. JetPack/L4T 버전에 맞는 ZED SDK installer를 Stereolabs release page에서 받아 설치한 뒤, `/usr/local/zed/get_python_api.py`가 생겼는지 확인합니다.
...
- 따라서 `cv2 + numpy==1.26.4` offline 분석 환경과 `pyzed + numpy==2.x` recording 환경을 분리하는 편이 안전합니다.
