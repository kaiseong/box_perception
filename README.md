...
현재 `KETI/pallet_box.png` 기준 오프라인 출력의 핵심 값:
...
별도 package install은 아직 없습니다. repository를 `/home/kgs/workspace`에 둔 상태에서 바로 실행합니다.

```bash
cd /home/kgs/workspace
python KETI/demo_offline.py \
  --image KETI/pallet_box.png \
  --save-debug KETI/pallet_box_debug.png \
  --mask-output KETI/pallet_box_mask.png
```
...
- `KETI/pallet_box_debug.png` 생성
- `KETI/pallet_box_mask.png` 생성
...
```bash
cd /home/kgs/workspace
python -m py_compile KETI/demo_offline.py KETI/recording.py KETI/box_pose/*.py KETI/tests/*.py
python -m unittest discover -s KETI/tests -v
```
...
python KETI/recording.py \
  --output-root KETI/recordings \
...
session = Path("KETI/recordings/desk_a_box_static_001")
...
image = cv2.imread("KETI/pallet_box.png", cv2.IMREAD_COLOR)
mask, stats = segment_yellow_box(image)
...
python -m py_compile KETI/demo_offline.py KETI/recording.py KETI/box_pose/*.py KETI/tests/*.py
python -m unittest discover -s KETI/tests -v
python KETI/demo_offline.py --image KETI/pallet_box.png --save-debug KETI/pallet_box_debug.png --mask-output KETI/pallet_box_mask.png
```
...
`KETI/tests/test_box_pose.py`는 다음 항목을 검증합니다.
