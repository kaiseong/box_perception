...
현재 1차 목표는 **D405 RGB-D 녹화 데이터를 replay해서 cropped box에서도 중심과 yaw를 추정**하는 것입니다. 구현된 기본 추정 경로는 학습 모델 없이 HSV 색상, rim/edge line, D405 depth band, 박스 실측 크기를 조합하는 `known_size` estimator입니다.
...
잘린 RGB mask의 bbox/OBB 중심은 실제 박스 중심이 아닐 수 있습니다. 그래서 replay의 기본 결과는 단순 `minAreaRect(mask)`가 아니라 `known_size`를 봅니다.

```text
D405 RGB
  -> yellow/orange mask
  -> dominant OBB는 debug/baseline으로만 유지
...
D405 depth
  ...
  -> 노란 evidence 중 박스 거리대만 남기는 filter로 사용
  -> depth_reference_m으로 center_top_camera_m을 임시 backprojection
```

D405 depth는 너무 가깝거나 멀 때, 얇은 테두리/반사/구멍/내부 물품 때문에 흔들릴 수 있습니다. 따라서 현재 구현은 depth point cloud OBB를 주 경로로 쓰지 않습니다. `metric` 결과는 비교용으로 남겨두지만, cropped D405 데이터에서는 `known_size`가 우선입니다.
...
- `known_size`: 현재 주 경로. all yellow evidence를 depth band로 정리한 뒤 Hough rim line과 박스 실측 크기로 center/yaw를 추정합니다.
...
- `known_size_still_frame_spread`

현재 로컬 D405 녹화 5개 세션을 `--stride 3 --view-rotation cw90`로 replay한 결과, `known_size_ok_fraction`은 모두 `1.0`입니다. yaw는 기존 `pixel`이 전부 fail이고 `metric` yaw가 크게 흔들리는 반면, `known_size`가 박스 rim 방향을 안정적으로 잡습니다.

대표 결과:

```text
d405_center_visible  known_size_ok=1.00  yaw_spread=5.03 deg
d405_left_crop       known_size_ok=1.00  yaw_spread=3.57 deg
d405_right_crop      known_size_ok=1.00  yaw_spread=7.49 deg
d405_yaw_plus        known_size_ok=1.00  yaw_spread=27.42 deg
d405_yaw_minus       known_size_ok=1.00  yaw_spread=23.16 deg
```
...
3. `known_size.center_image` ray를 calibrated top plane과 교차시켜 T5 center를 만듭니다.
4. image yaw를 T5 yaw로 변환하는 기준축을 정의합니다.
5. 같은 정지 pose에서 여러 frame을 replay해 center spread가 `2 cm` 이하인지 확인합니다.
6. yaw가 고정 pose에서 `4 deg` 이하로 흔들리는지 확인합니다.

학습 기반 segmentation(YOLO 등)은 geometry path가 실제 책상/조명/장갑 조건에서 계속 실패하는 케이스가 확인된 뒤 fallback으로 추가합니다. 현재 데이터에서는 색상 + depth band + known-size fitting이 먼저 시도할 만한 경로입니다.
