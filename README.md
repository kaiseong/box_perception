...
Jetson에서는 RealSense Python wheel 호환성을 위해 **Python 3.12 venv를 권장**합니다. `python -V`가 `Python 3.13.x`로 나오면 Jetson aarch64용 `pyrealsense2` wheel이 잡히지 않을 수 있으므로 venv를 3.12로 다시 만듭니다.
...
`python -V`는 `Python 3.12.x`여야 합니다. 이미 `.venvs` 같은 다른 이름의 venv를 쓰고 있어도 상관없지만, 그 venv가 Python 3.13이면 새 Python 3.12 venv를 만들어야 합니다.
...
이 경우 apt로는 device/udev/runtime 쪽을 설치하고, venv 안에서는 공식 PyPI wheel을 먼저 설치합니다. 현재 Python 3.12 aarch64 wheel이 제공되므로 이 경로가 가장 가볍습니다. `python -V`가 `Python 3.13.x`이면 `pip install pyrealsense2`가 `No matching distribution found`로 실패할 수 있습니다.
...
예를 들어 `/usr/local/lib/python3.12/pyrealsense2` 아래에 설치되어 있으면:

```bash
export PYTHONPATH="${PYTHONPATH}:/usr/local/lib/python3.12/pyrealsense2"
```
