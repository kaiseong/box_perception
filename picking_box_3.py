#!/usr/bin/env python3
"""Vision-adjusted pre-pick motion with mobile-base alignment for RBY1 M.
...
Default sequence:
  ...
  3. mobile_base_align               (M-model SE(2), closed loop, x/y only)
...
# ---- Mobile base closed-loop alignment (picking_box_3 only) ----
# M model supports SE(2) x/y velocity commands. The control law is deliberately
# conservative: use vision, move a small amount, then look again. If alignment
# cannot converge, the latest fresh vision measurement is still used for the
# normal arm-only vision_pre_push stage.
MOBILE_BASE_ALIGN_DEFAULT = True
MOBILE_BASE_TARGET_X_M = 0.45
MOBILE_BASE_X_TOLERANCE_M = 0.01
MOBILE_BASE_Y_TOLERANCE_M = 0.01
MOBILE_BASE_MAX_SPEED_MPS = 0.02
...
def setup_max_open_gripper(robot: Any) -> MaxOpenGripper | None:
    ...
    print_stage("0/6 gripper_home_open", "setting tool voltage")
    ...
    if not gripper.set_open_target():
        return None
    gripper.start()
    print_stage("0/6 gripper_home_open", f"max-open hold started; min={gripper.min_q}, max={gripper.max_q}")
    return gripper
...
class ContinuousLiveBoxView:
    ...
    def __init__(self, view_rotation: str) -> None:
        ...
        self.window_name = "picking_box_3 mobile align vision (q/ESC abort)"
        self._closed = False
        ...
        *,
        ...
        while len(centers) < int(frames_needed):
            ...
            try:
                ...
                return None
            if abort_requested:
                print_stage("3/6 mobile_base_align", "visualization abort requested")
                raise UserAbortRequested("mobile alignment recapture aborted from the live view")
            if usable:
                centers.append(estimate.center_top_camera_m)
                modes.append(mode)
                axis = estimate.support.get("long_axis_camera")
    ...
    *,
    ...
    try:
        for iteration in range(1, int(max_iterations) + 1):
            ...
            if remaining <= 0.0:
                print_stage("3/6 mobile_base_align", "timeout; continuing with latest vision measurement")
                return latest
            ...
            if float(np.linalg.norm(velocity_xy)) <= 1e-6:
                print_stage("3/6 mobile_base_align", "zero correction; continuing with latest vision measurement")
                return latest
            ...
            remaining = deadline - time.monotonic()
            if remaining <= 0.0:
                print_stage("3/6 mobile_base_align", "timeout after move; recapture required")
                return None
            ...
            if refreshed is None:
                print_stage("3/6 mobile_base_align", "post-move vision failed; recapture required")
                return None
            latest = refreshed
    ...
    try:
        if not visualize_only and gripper_open:
            ...
            if gripper_device is None:
                print_stage("0/6 gripper_home_open", "FAILED; aborting before arm motion")
                return False
        elif not visualize_only:
            print_stage("0/6 gripper_home_open", "skipped by --no-gripper-open")
        ...
        if box_center_camera_m is None:
            ...
            print_stage("2/6 live_vision", "capturing D405 frames")
            ...
            print_stage("2/6 live_vision", "skipped; using supplied center_top_camera_m")
            q = robot.get_state().position
            camera_to_base = compute_camera_to_base_for_view_rotation(
                view_rotation,
                dyn_model,
        ...
        print_stage("4/6 vision_pre_push", "building target")
        q = robot.get_state().position
        ...
        if not send_stage(
            ...
            print_stage("4/6 vision_pre_push", "FAILED; aborting")
            return done
        print_stage("4/6 vision_pre_push", "reached")
        ...
        if not send_stage(
            ...
            print_stage("6/6 lift", "FAILED; aborting")
            return done
        print_stage("6/6 lift", "done")
        ...
        if gripper_device is not None:
            print_stage("0/6 gripper_home_open", "stopping max-open background loop")
            gripper_device.stop()
